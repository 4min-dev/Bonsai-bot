...
async function getAllUsersData(isPreloaderIgnor) {
    try {
        if (!isPreloaderIgnor) {
            preloader.setActive()
        }
...
async function getProfileData(isPreloaderIgnor) {

    try {
        const userData = await getUserData(isPreloaderIgnor)
        const usersData = await getAllUsersData(isPreloaderIgnor)
